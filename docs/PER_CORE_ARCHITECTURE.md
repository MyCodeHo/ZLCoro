...
**最新优化**：采用 **SO_REUSEPORT 每核独立 Accept** 模式，消除惊群效应。
...
| **无惊群效应** | SO_REUSEPORT 让内核直接分派连接，只唤醒目标核心 |
| **极低延迟** | 无队列等待，事件就绪立即处理 |
| **缓存友好** | Accept 和处理在同一核心，数据不跨核移动 |
| **线性扩展** | 性能随核心数近乎线性增长 |
| **确定性** | 同一连接总是在同一核心处理 |
...
- **HTTP Hello World**: ~98 万 QPS（epoll 后端）
- **Echo 服务器**: ~100 万 QPS
- **优化前后对比**: 87 万 → 98 万 QPS（+13%）
