...
private:
    ...
    /// @brief 可用内存块列表
    /// @details 存储已分配但未使用的内存块指针。
    ///          acquire() 从末尾取出（LIFO），release() 向末尾添加。
    ///          LIFO 策略有利于缓存局部性（最近释放的块可能还在缓存中）。
    /// @thread_safety 由 lock_ 保护
    std::vector<void*> pool_;
    
    /// @brief 自旋锁
    /// @details 保护 pool_ 的并发访问。
    ///          使用自旋锁而非 mutex 是因为临界区很短，避免上下文切换开销。
    mutable SpinLock lock_;
    
    /// @brief 池的最大容量
    /// @details 限制缓存的内存块数量，超出时直接释放。
    ///          防止内存无限增长。
    size_t max_size_;
};
...
private:
    ...
    /// @brief 管理的对象指针
    /// @details RAII 管理的资源，析构时自动归还到池。
    ///          nullptr 表示空指针（已释放或未初始化）。
    T* ptr_;
    
    /// @brief 对象所属的池
    /// @details 用于在析构或 reset() 时归还对象。
    ///          如果为 nullptr，reset() 不会执行任何操作。
    ObjectPool<T>* pool_;
};
...
private:
    ...
    /// @brief 每个内存块的大小（对齐后）
    /// @details 在构造时根据用户指定值向上对齐到 max_align_t 边界。
    ///          之后不可修改。
    size_t block_size_;
    
    /// @brief 空闲内存块列表
    /// @details 存储可立即分配的内存块指针。
    ///          allocate() 从末尾取出，deallocate() 向末尾添加。
    /// @thread_safety 由 mutex_ 保护
    std::vector<void*> free_list_;
    
    /// @brief 已分配的大块（chunk）列表
    /// @details 每个 chunk 包含 32 个固定大小的块。
    ///          记录所有 chunk 用于析构时统一释放。
    /// @thread_safety 由 mutex_ 保护
    std::vector<void*> chunks_;
    
    /// @brief 互斥锁
    /// @details 保护 free_list_ 和 chunks_ 的并发访问。
    std::mutex mutex_;
};
