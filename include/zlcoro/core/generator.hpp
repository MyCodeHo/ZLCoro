...
public:
    ...
    private:
        ...
        /// @brief 用于存储右值的联合体
        /// @details 使用 union 实现延迟初始化，避免 T 的默认构造要求。
        ///          只有当 co_yield 传入右值时才会在此构造。
        ///          左值直接存储指针，无需拷贝。
        union {
            T stored_value_;  ///< 右值的存储位置（通过移动构造）
        };
        
        /// @brief 指向当前 yield 值的指针
        /// @details 可能指向两个位置：
        ///          1. stored_value_（右值情况）
        ///          2. 协程帧中的变量（左值情况，生命周期安全）
        ///          这种设计避免了不必要的拷贝。
        const T* value_ptr_;
        
        /// @brief 协程执行过程中未捕获的异常
        /// @details 在迭代时会重新抛出，保证异常正确传播。
    ...
    private:
        ...
        /// @brief 关联的协程句柄
        /// @details 通过此句柄控制协程的恢复和获取 yield 的值。
        ///          默认构造时为空（表示 end 迭代器）。
...
private:
    ...
    /// @brief 底层协程句柄
    /// @details 持有生成器协程帧的所有权。协程帧包含：
    ///          - Promise 对象（包含当前 yield 的值）
    ///          - 协程的局部变量
    ///          - 协程的挂起点
    ///          Generator 移动时转移所有权，析构时销毁协程帧。
    /// @ownership Generator 独占所有权（RAII）
    std::coroutine_handle<promise_type> handle_;
};
